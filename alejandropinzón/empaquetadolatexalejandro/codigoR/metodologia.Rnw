...
\section{METODOLOGÍA}
	...
	<<datosep, results='asis', echo=FALSE, warning=FALSE, include=FALSE, menssage=FALSE>>=
		options(java.parameters="-Xmx1024m");
		datosep <- read.csv2("../DataSet/datosep.csv")
		datosept <- read.csv2("../DataSet/datosept.csv")
	...
	#	library(scatterplot)	
		...
		datos_estrato <- as.character(datosep$estrato)
		datos_promedio <- datosep$promedio
		datos01<- subset(datosep, datosep$promedio>=0 & datosep$promedio<=1)
		datos12<- subset(datosep, datosep$promedio>=1.1 & datosep$promedio<=2)
		datos23<- subset(datosep, datosep$promedio>=2.1 & datosep$promedio<=3)		
		datos34<- subset(datosep, datosep$promedio>=3.1 & datosep$promedio<=4)
		datos45<- subset(datosep, datosep$promedio>=4.1 & datosep$promedio<=5)
 ...
 El dataset utilizado en este análisis fue obtenido mediante una consulta sobre la base de datos de la Universidad de los Llanos y esta conformada por la información de los estudiantes activos de todas las facultades que componen la universidad.
  ...
  \begin{itemize}
	...
	\begin{figure}[ht]
		\centering
		<<estrato, results='asis', width=5, warning=FALSE >>=
			with(datosep, hist(estrato, breaks="Sturges", col="darkgray", main="Diagrama de barras"))
		@	
	...
	El promedio ponderado de carrera es una variable de carácter cuantitativo de tipo continuo, a la que se le puede aplicar la media y la mediana como medidas de tendencia central, a continuación se presentan los resultados.
		...
		<<tabla_promedio, results='asis'>>=
			dfull <- data.frame(Estrato=datosep$estrato)
			stargazer(dfull, type="latex", title="Promedio academico ponderado")
		@
...
%	\end{figure}
	...
	\begin{figure}[ht]
		\centering
		<<densidad, results='asis'>>=
			densityplot( ~ promedio, data=datosep, bw="SJ", adjust=1, kernel="epanechnikov",  main="Diagrama de estimación de densidad")
			#densityplot( ~ promedio, data=datosep, bw="SJ", adjust=1, kernel="biweight")
			#densityplot( ~ promedio, data=datosep, bw="SJ", adjust=1, kernel="gaussian")
		@
	...
	\begin{figure}[ht]
		\centering
		<<diagrama_puntos, results='asis'>>=
			stripchart(datosep$promedio, method="stack", xlab="Promedio ponderado Academico", main="Diagrama de caja")
		@
	...
	\begin{center}
		\bigskip
		<<shapiro, results='asis'>>=						
			with(datosep, shapiro.test(promedio))
		@
	\end{center}
	...
	\begin{figure}[ht]
		\centering
		<<boxplot, results='asis'>>=
			boxplot(datosep$promedio, data=datosep, xlab="Promedio Ponderado Academico", ylab="Rango de calificación", id.method="y", main="Diagrama de caja")
		@
	...
	\begin{center}
		<<shapiro_truncado, results='asis'>>=						
			with(datosept, shapiro.test(promedio))
		@
	\end{center}
	\bigskip	
	...
	\begin{figure}[ht]
		\centering
			<<boxplot_trucado, results='asis'>>=
				boxplot(datosept$promedio, data=datosept, xlab="Promedio Ponderado Academico", ylab="Rango de calificación", id.method="y", main="Diagrama de caja")
			@
	...
	\begin{figure}[ht]
		\centering
		<<dispersion, results='asis'>>=
			xyplot(promedio ~ estrato, type="p", pch=16, auto.key=list(border=TRUE), par.settings=simpleTheme(pch=16), scales=list(x=list(relation='same'), y=list(relation='same')), data=datosep)
			
			#scatterplot(datosep$promedio~datosep$estrato, reg.line=lm, smooth=TRUE, spread=TRUE, id.method='mahal', id.n = 2, boxplots='xy', span=0.5, data=datosep, main="Estimación de densidad")
			
			#scatterplot(promedio~estrato, reg.line=lm, smooth=TRUE, spread=TRUE, id.method='mahal', id.n = 2, boxplots='xy', span=0.5, data=datosep, main="Estimación de densidad")
